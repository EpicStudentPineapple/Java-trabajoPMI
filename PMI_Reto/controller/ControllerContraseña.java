	...
	public ControllerContraseña(VistaContraseña vista, VistaLogin vistaLogin) {
		...
		vista.getBtnRestablecer().addActionListener(e -> {
			...
			if (!nuevaContraseña.equals(confirmarContraseña)) {
				JOptionPane.showMessageDialog(vista, "Las contraseñas no coinciden.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (usuarioExiste) {
				...
				if (actualizado) {
					JOptionPane.showMessageDialog(vista, "Contraseña actualizada correctamente.", "Éxito",
							JOptionPane.INFORMATION_MESSAGE);
					vista.cerrar();
					vistaLogin.iniciar();
				} else {
					JOptionPane.showMessageDialog(vista, "No se pudo actualizar la contraseña.", "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(vista, "El usuario con ese DNI no existe.", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		});
	}
